...
export async function createSession(db: D1Database, userId: string, now = nowSeconds()) {
  ...
  await db
    .prepare(
      `INSERT INTO user_sessions (token_hash, user_id, expires_at, created_at, last_used_at)
       VALUES (?, ?, ?, ?, ?)`,
    )
...
export async function cleanupExpiredAuthRecords(db: D1Database, now = nowSeconds()) {
  await db.batch([
    db
      .prepare(
        'DELETE FROM oauth_login_attempts WHERE expires_at <= ? OR consumed_at IS NOT NULL',
      )
      .bind(now),
    db
      .prepare(
