...
export const videoR2Handler: OutboundHandler<
...
> = async (request, env, ctx) => {
  ...
  if (request.method === 'PUT' && pathname === '/output') {
    ...
    try {
      const written = await env.VIDEOS.put(storage.output_r2_key, request.body, {
      ...
      if (!written) {
        const winner = await env.VIDEOS.head(storage.output_r2_key);
        return winner?.customMetadata?.sha256 === checksum
          ? new Response(null, {status: 204})
          : new Response('Immutable derivative already exists', {status: 409});
      }
