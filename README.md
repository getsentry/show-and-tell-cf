...
`src/worker/sentry.ts` configures 10% tracing, Cloudflare version IDs as releases, and explicit v11 `dataCollection` limits: no user identity, cookies, HTTP headers/bodies, query parameters, or database values. Error messages and stacks still reach Sentry; do not attach video contents, credentials, or email bodies to exceptions. Logs, replay, and source-map uploads to Sentry are not enabled. Source-map uploads require separate build credentials/configuration.
...
The home page lists shows as text-and-CSS cards with a prominent **Open the screening** link and a smaller **Upload & submissions** link. It does not automatically open a show. Admins can expand **New playlist** on the overview; its title defaults to `Show & Tell {Month} - {Year}` using the browser's current local month. Creating it adds it to the list and opens its submission page.

Admins can open **Edit playlist** from an overview card or a playlist's submission page to update its title and description. **Save changes** keeps share links, visibility, schedule, submissions, and videos unchanged; **Cancel** discards edits. Metadata updates use admin-only `PUT /api/events/:id` with `title` and `description`. Edits are blocked while a reminder is sending or uncertain; already-sent reminders are not updated or resent.

The edit view also offers **Move to trash**, with confirmation. **View trash** on the overview lists removed playlists and offers **Restore playlist**. There is no permanent playlist deletion: trash preserves submissions, order, visibility, upload records, and stored video objects. It blocks direct playlist links, new submissions, video uploads, and playback requests for everyone until restored; already buffered media cannot be revoked. In-flight video processing can finish and incomplete uploads retain their existing expiry policy.

Trash pauses automatic reminders and reveal. Restore preserves hidden/canceled status and resumes reminders only under the existing schedule and 24-hour catch-up rules, without resending delivered mail. A sending or uncertain reminder must be reconciled before trashing. API: admin-only `GET /api/events?trash=true`, `POST /api/events/:id/trash`, and `POST /api/events/:id/restore`, using the existing session and same-origin protections. Apply migration `0011_playlist_trash.sql` before deploying this feature; no production data is changed by local checks.

Share `/events/<event-id>` for submissions or `/playlists/<event-id>` for screening. Both destinations survive Google sign-in through the same browser-bound, allowlisted OAuth flow. The submission page has **Copy submission link**; the player has **Copy screening link**. Legacy `/?event=<event-id>` links still open the submission page and normalize to the new path for sign-in.
...
Members see only their own non-deleted submissions under **Your submissions**, including entries hidden from the show. Overview and playlist-switcher counts reflect their own submissions. Admins see all non-deleted submissions and retain **Arrange playlist**; switching to user view applies the member filter. The Worker filters both submission lists and counts, rather than hiding other users' entries only in the browser. The shared screening remains unchanged: everyone can watch ready, non-hidden, non-deleted videos in the admin-defined order. This simplifies upload management; it does not make submitted videos private.
