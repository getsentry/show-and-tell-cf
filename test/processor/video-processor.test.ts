...
if (!pinnedTools.ok)
  throw new Error(
    'Pinned FFmpeg 8.0.1 tools required; run npm run test:processor:docker',
  );
...
describe('video processor canonicalization', () => {
  ...
  it('generates an AAC silence track for silent video', async () => {
    ...
    const processed = await run('node', [processorPath, 'process-file', input, output]);
    expect(parseJsonObject(processed.stdout)).toMatchObject({
      videoCodec: 'h264',
      audioCodec: 'aac',
      audioMode: 'generated-silence',
      fastStart: true,
    });
  }, 120_000);

  it('rejects corrupt media without publishing output metadata', async () => {
    const directory = await createTempDir();
    const input = path.join(directory, 'corrupt.mp4');
  ...
  it('rejects videos longer than ten minutes before transcoding', async () => {
    const directory = await createTempDir();
...
async function detectPinnedTools() {
  try {
    ...
    const hasFfmpeg8 = ffmpegVersion.stdout.includes('ffmpeg version 8.0.1');
    const hasFfprobe8 = ffprobeVersion.stdout.includes('ffprobe version 8.0.1');
...
async function createTempDir() {
  const directory = await mkdtemp(path.join(tmpdir(), 'show-and-tell-processor-'));
